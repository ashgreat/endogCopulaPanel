...
are the residuals of `y = alpha_i + x'beta + z'delta + e` on the
original rows; with `structural = FALSE` they are the residuals of the
FOD-transformed model on the transformed rows, i.e. the ones the
likelihood treats as `N(0, sigma^2)`.

## Usage

``` r
# S3 method for class 'copregpanel'
residuals(object, structural = TRUE, ...)
```
...
- structural:
  ...
  Logical; `TRUE` (the default) for the residuals of the structural
  model, `FALSE` for the residuals of the FOD-transformed model.
...
A named numeric vector of residuals.
